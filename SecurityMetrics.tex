...
$a_1 =
\begin{cases}
	0 & \text{if } id(ARFCN_{curr}) = \varnothing \vee id(ARFCN_{curr}) = LAC,CID_{curr} \\
	1 & \text{otherwise} \\
\end{cases}$
...
% - What do you mean with "no service specified"
% - How do we measure that the transaction was released with no SMS/call happening?
% - What do we need to check for being absent/present?
% - What is the PAGING REQUEST timestamp needed for?
...
\colorlet{inprog}{srlyellow}
\colorlet{avail}{srlgreen}

\renewcommand{\arraystretch}{1.5}

\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}} p{0.6\linewidth} lp{0.1\linewidth}p{0.1\linewidth}}
{\bf Information}                                & {\bf Criterion}      & {\bf Samples}       & {\bf Phone}         \\\hline
CID of serving cell                              & A1                   & \cellcolor{srldark} & \cellcolor{srldark} \\
LAC of serving cell                              & A1, A2               & \cellcolor{srldark} & \cellcolor{srldark} \\
LACs of all neighboring cells                    & A2                   & \cellcolor{srldark} & \cellcolor{srldark} \\
BA list of serving cell                          & A2, K1               & \cellcolor{srldark} & \cellcolor{srldark} \\
BA lists of neighboring cells                    & R1                   & \cellcolor{srldark} & \cellcolor{srldark} \\
ARFCN of serving cell                            & A1, R1               & \cellcolor{srldark} & \cellcolor{srldark} \\
3G/4G availability                               & A3                   & \cellcolor{srldark} & \cellcolor{srldark} \\
CELL RESELECT OFFSET                             & K2                   & \cellcolor{srldark} & \cellcolor{srldark} \\
Encryption algorithm in use                      & C1                   & \cellcolor{srldark} & \cellcolor{srldark} \\
Timestamp of first CIPHER MODE COMPLETE message  & C2                   & \cellcolor{srldark} & \cellcolor{srldark} \\
Timestamp of first encrypted message             & C2                   & \cellcolor{srldark} & \cellcolor{srldark} \\
Cipher Response field of CIPHER MODE COMMAND     & C3                   & \cellcolor{srldark} & \cellcolor{srldark} \\
Last LOCATION UPDATE REQUEST                     & C4                   & \cellcolor{srldark} & \cellcolor{srldark} \\
Cause of LOCATION UPDATE REJECT (if applicable)  & C4                   & \cellcolor{srldark} & \cellcolor{srldark} \\
Initial value of T3212                           & T1                   & \cellcolor{srldark} & \cellcolor{srldark} \\
Timestamp of PAGING REQUEST                      & T3                   & \cellcolor{srldark} & \cellcolor{srldark} \\
Mobile identity type of PAGING REQUEST           & T3                   & \cellcolor{srldark} & \cellcolor{srldark} \\
Cause of CHANNEL RELEASE                         & T3                   & \cellcolor{srldark} & \cellcolor{srldark} \\
Tx power level                                   & T7                   & \cellcolor{srldark} & \cellcolor{srldark} \\
BS\_AG\_BLKS\_RES                                & R2                   & \cellcolor{srldark} & \cellcolor{srldark} \\
BS\_CCCH\_SDCCH\_COMB                            & R2                   & \cellcolor{srldark} & \cellcolor{srldark} \\
BS\_PA\_MFRRS                                    & R2                   & \cellcolor{srldark} & \cellcolor{srldark} \\
\bottomrule
\end{tabular*}
...
\cellcolor{black}      & missing &
\cellcolor{srlgrey}    & unknown &
\cellcolor{srlred}     & research &
\cellcolor{srlyellow}  & in progress &
\cellcolor{srlgreen}   & available \\
\end{tabular*}
