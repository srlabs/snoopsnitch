...
this makes no sense. If the catcher announces neighboring cells, but prevents
transition to them e.g. by sending suitable cell reselection parameters, most
of those real cells likely have a different LAC. Trouble is, that neighbor list
just consist of ARFCN and the LAC needs to be read from the respective BCCHs.
Unfortunately on Qualcomm we cannot correlate the LAC/CID data with the ARFCN
it was received from an thus may not be able to obtain this information.}
...
surrounding cells and with a much better signal strength than the other cells.
If the IMSI catcher announces realistic neighboring cells, this difference
between the LAC of the serving cell and all neighboring cell could be detected.
...
\FIXME{Empirically derive a function $p(N)$ that estimates the likelihood that
$N$ of a cells neighboring stations have a different LAC from the GSMmap data.}
...
\subsubsection{\critt{6}{RACH requests not acknowledged}}
...
\subsubsection{\critr{3}{High PCH downlink error rate}}
...
DSC value of the MS (\FIXME{maybe history on the DSC})
