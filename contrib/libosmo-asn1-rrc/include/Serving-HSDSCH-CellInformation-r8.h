/*
 * Generated by asn1c-0.9.24 (http://lionet.info/asn1c)
 * From ASN.1 module "InformationElements"
 * 	found in "../asn/InformationElements.asn"
 * 	`asn1c -fcompound-names -fnative-types`
 */

#ifndef	_Serving_HSDSCH_CellInformation_r8_H_
#define	_Serving_HSDSCH_CellInformation_r8_H_


#include <asn_application.h>

/* Including external dependencies */
#include "DeltaACK.h"
#include "DeltaNACK.h"
#include "HARQ-Preamble-Mode.h"
#include <NativeEnumerated.h>
#include <constr_SEQUENCE.h>

#ifdef __cplusplus
extern "C" {
#endif

/* Dependencies */
typedef enum Serving_HSDSCH_CellInformation_r8__mac_hsResetIndicator {
	Serving_HSDSCH_CellInformation_r8__mac_hsResetIndicator_true	= 0
} e_Serving_HSDSCH_CellInformation_r8__mac_hsResetIndicator;

/* Forward declarations */
struct PrimaryCPICH_Info;
struct DL_HSPDSCH_Information_r8;
struct HARQ_Info_r7;

/* Serving-HSDSCH-CellInformation-r8 */
typedef struct Serving_HSDSCH_CellInformation_r8 {
	DeltaACK_t	*deltaACK	/* OPTIONAL */;
	DeltaNACK_t	*deltaNACK	/* OPTIONAL */;
	HARQ_Preamble_Mode_t	 harq_Preamble_Mode;
	struct PrimaryCPICH_Info	*primaryCPICH_Info	/* OPTIONAL */;
	struct DL_HSPDSCH_Information_r8	*dl_hspdsch_Information	/* OPTIONAL */;
	struct HARQ_Info_r7	*harqInfo	/* OPTIONAL */;
	long	*mac_hsResetIndicator	/* OPTIONAL */;
	
	/* Context for parsing across buffer boundaries */
	asn_struct_ctx_t _asn_ctx;
} Serving_HSDSCH_CellInformation_r8_t;

/* Implementation */
/* extern asn_TYPE_descriptor_t asn_DEF_mac_hsResetIndicator_8;	// (Use -fall-defs-global to expose) */
extern asn_TYPE_descriptor_t asn_DEF_Serving_HSDSCH_CellInformation_r8;

#ifdef __cplusplus
}
#endif

/* Referred external types */
#include "PrimaryCPICH-Info.h"
#include "DL-HSPDSCH-Information-r8.h"
#include "HARQ-Info-r7.h"

#endif	/* _Serving_HSDSCH_CellInformation_r8_H_ */
#include <asn_internal.h>
